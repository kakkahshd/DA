...
## Session log — Iter 213 (2026-09-16): PEMULIHAN container dari GitHub `pandeyoga/DAHOST` + analisis "data yang harus diisi" (tanpa perubahan kode)
- Repo di-clone ke `/app`, `backend/.env` dibuat (JWT_SECRET baru, ALLOW_DEMO_SEED=false, SEED_DEFAULT_LOCATIONS=false, FILE_STORAGE=local), deps pip/yarn terpasang, `frontend/build` dibangun (static_server), backend healthy, login admin OK.
- Catatan deps: pin `emergentintegrations==0.2.0` vs `litellm 1.80.0` di requirements.txt konflik di pip resolver → dipasang tanpa dua baris itu (keduanya sudah ada di image). Tidak mengubah requirements.txt.
- **Temuan penting**: DB preview KOSONG dari data klien (hanya seed startup: COA 201, kas/bank 20, roles). Workbook klien `MASTER_DATA_DA_PERBAIKAN*.xlsx` dan snapshot `dahost_snapshot_2026-09-13.archive.gz` TIDAK ada di repo (gitignore `/private/`, `/deploy/golive/*`, `/uploads/`). Untuk melanjutkan pekerjaan berbasis data (papan kelengkapan, template isi massal) perlu salah satu: snapshot dari VPS (`deploy/backup.sh`/mongodump `dahost_erp`) atau workbook klien + `golive.sh apply`.
